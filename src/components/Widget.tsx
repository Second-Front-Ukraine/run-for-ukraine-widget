    ...
    <div className="sfua-widget">
      {tab ? tab.paid ? (
        <div className="text-center">
          <p className="lead">{props.lang === 'uk' ? "Дякуємо за Вашу реєстрацію!" : "Thank You for the registration!"}</p>
          <p className="lead">💙&nbsp;💛</p>
          <h2>{props.lang === 'uk' ? "Погнали!" : "Let's go!"}</h2>
          <small><a href="#" className="m-1 btn btn-sm btn-light" onClick={handleDonationCancel}>{props.lang === 'uk' ? "Нова реєстрація" : "New registration"}</a></small>
        </div>
      ) : props.lang === 'uk' ? (
        <div className="text-center">
        ...
        <div className="text-center">
          <p className="lead">Pending <a href="#" onClick={handleClickDonation}>payment</a> in a new browser window.<br /> <em>If new browser window didn't open, click "Pay now"</em>.
          </p>
          <a href="#" className="m-1 btn btn-light" onClick={handleClickDonation}>Pay now</a>
          <a href="#" className="m-1 btn btn-sm btn-light" onClick={handleDonationCancel}>Cancel</a>
        </div>
      )
        : (
          <div>
            <DonateForm campaign={props.campaign} lang={props.lang} onTabCreated={onTabCreated} />
          </div>
        )}
    </div>
  );
